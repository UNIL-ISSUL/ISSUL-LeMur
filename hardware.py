#This file is responsible to provide an interface to hardware
#This project use a revolution pi module with AIO and DIO extension as a PLC
#There is also a MODBUS Communication to communicate with periphirals
#Configuration of the treadmill is stored in a YAML file
...
#split high and low byte of register
...
class revPI() :
    ...
    def stop_lift(self,msg="") :
        ...
        Logger.info(f"STOP lift, reason : {msg}")
    
    #define a new lift angle to PId via modbus variable
    def set_lift_angle(self,angle) :
        self.rpi.io.lift_angle_SP.value = round(angle * 100)
        Logger.info(f"SET lift angle to : {angle}")

    #return current lift angle
    def get_lift_angle(self) :
        return float(self.rpi.io.lift_angle_current.value/100)

    #enable PID on external controller
    ...
    def set_belt_speed(self,V_kmh) :
        ...
        value = value * 1.8 #new gearbox ratio 4:1
